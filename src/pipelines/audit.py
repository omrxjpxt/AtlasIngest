...
async def run_audit():
    ...
    async with get_session() as session:
        ...
        # --- JOBS AUDIT ---
        stmt_j = select(func.count(Job.id))
        total_jobs = (await session.execute(stmt_j)).scalar()

        # --- NEWS AUDIT ---
        stmt_n = select(func.count(News.id))
        total_news = (await session.execute(stmt_n)).scalar()

        logger.info("=== JOBS AUDIT RESULTS ===")
        logger.info(f"Total Jobs in DB: {total_jobs}")
        logger.info("==============================")

        logger.info("=== NEWS AUDIT RESULTS ===")
        logger.info(f"Total News in DB: {total_news}")
        logger.info("==============================")
